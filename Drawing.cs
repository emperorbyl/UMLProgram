...
namespace UMLProgram
{
    public class Drawing
    {
        private static readonly DataContractJsonSerializer JsonSerializer =
                new DataContractJsonSerializer(typeof(List<Element>), new[] { typeof(Element), typeof(SymbolWithAllState), typeof(SymbolExtrinsicState) });
        ...
        public void LoadFromStream(Stream stream)
        {
            ...
            lock (_myLock)
            {
                ...
                {
                    var tmpTree = element as SymbolWithAllState;
                    if (tmpTree != null)
                    {
                        Symbol fullTree = SymbolFactory.Instance.GetSymbol(tmpTree.ExtrinsicState);
